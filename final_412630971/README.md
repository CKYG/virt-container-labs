...
builder stage只負責建置和安裝套件，所以最後不會成為實際執行的image。runtime stage只會把需要的檔案從builder複製過來，所以image比較小。但builder的中間層還是可能保存在Docker的快取中，所以可以用docker images -a查看看。
![multistage-size-run](screenshots/multistage-size-run.png)
